  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-5">
        ...
        <span
          className={`font-mono text-[11px] px-2.5 py-0.5 border rounded-sm ${categoryColors[project.category] || 'bg-accent/10 text-accent border-accent/20'}`}
        >
          {project.category}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-6">
        {project.techStack.slice(0, 4).map((tech) => (
          <span
            key={tech}
            className="font-mono text-[11px] text-muted px-2 py-0.5 bg-white/[0.03] border border-white/[0.06]"
          >
            {tech}
          </span>
        ))}
