...
export default function Navbar() {
  ...
  const { activeSection, mobileMenuOpen, isScrolled } = useAppSelector(
    (s) => s.ui
  );
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-between transition-all duration-300 backdrop-blur-md bg-bg/70 border-b border-border/20 ${isScrolled ? 'py-3 px-6 md:px-[60px]' : 'py-5 px-6 md:px-[60px]'}`}
    >
      <a
        href="#hero"
        className="font-mono text-[13px] text-accent tracking-[0.1em] no-underline"
        onClick={(e) => {
          e.preventDefault();
          scrollTo('#hero');
        }}
      ...
      <a
        href="/resume.pdf"
        className="hidden md:block font-mono text-[12px] text-accent border border-accent px-5 py-2 hover:bg-accent/10 transition-colors no-underline"
        target="_blank"
