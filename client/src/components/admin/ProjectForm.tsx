...
export default function ProjectForm({ project, onDone }: Props) {
  ...
  const FL =
    'block font-mono text-[11px] text-accent tracking-[0.12em] uppercase mb-2';
    ...
    >
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="What does this project do? Mention scale and outcome."
        />
        {errors.description && (
          <p className={FE}>{errors.description.message}</p>
        )}
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows={4}
          className={FI + ' resize-none'}
          placeholder="Full case study — problem, solution, results, key decisions."
        />
      </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {techStack.map((t) => (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => setTechStack((p) => p.filter((x) => x !== t))}
                  className="text-red-400 hover:text-red-300 text-[14px] leading-none"
                >
                  ×
                </button>
              </motion.span>
