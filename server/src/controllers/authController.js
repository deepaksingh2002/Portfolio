...
export const getMe = asyncHandler(async (req, res) => {
    return res.status(200).json(
        new ApiResponse(200, { admin: req.admin }, "Admin fetched")
    )
})
...
export const refreshAccessToken = asyncHandler(async (req, res) => {
    ...
    const { accessToken, refreshToken: newRefreshToken } = await generateTokens(admin._id)
...
export const seedAdmin = asyncHandler(async (req, res) => {
    ...
    if (existing) {
        ...
        if (!passwordOk) {
            ...
            await existing.save()
            return res.status(200).json(
                new ApiResponse(200, { email: SEED_EMAIL }, "Admin password reset to seed value")
            )
        }

        return res.status(200).json(
            new ApiResponse(200, { email: SEED_EMAIL }, "Admin already exists and credentials are valid")
        )
    }
    ...
    return res.status(201).json(
        new ApiResponse(201, { email: admin.email }, `Admin seeded — use password: ${SEED_PASS}`)
    )
})
